classdef node < torsche.schedobj
    properties
        parent
        Name
        UserParam
    ...
    methods
        ...
        if nargin==3
            if isa(varargin{1},'torsche.node') && ischar(varargin{2}) 
        ...
        na = nargin;
        if na>=1 && isa(varargin{1},'torsche.node'),
            obj = varargin{1};
            if na > 1,
        ...
        parent = torsche.schedobj;

        % Label task as an object of class TASK
        %node = class(node,'node', parent); 

        obj.GraphicParam = {torsche.node.getdefaultnode};
