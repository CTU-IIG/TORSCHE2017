...
classdef graph < torsche.schedobj
    properties
        parent
        Name
        E
    ...
    methods
        ...
        %--------------------------------------------------------------------------
            % from graph
            if nargin==3
                if isa(varargin{1},'torsche.graph') && ischar(varargin{2}) 
                    obj = set_helper(varargin{:});
                    return;
                end
            end

            if nargin >= 1 && isa(varargin{1},'torsche.graph'),
                        ...
                        for i = 2:2:length(varargin)                
                            ...
                            switch lower(varargin{i})
                                ...
                                case edgeListWords
                                    ...
                                    obj = torsche.graph.addedges(obj,edgeList);

                                case nodeListWords       
                                    nodeList = varargin{i+1};
                                    if (min([nodeList{:,1}]) < length(obj.N))
                                    ...
                                    obj = torsche.graph.addnodes(obj,nodeList);

                                case edgeDataTypesWords
                                    [isOK,E] = testofdatatypes(obj.E,varargin{i+1});
                                    if isOK
                                        obj.DataTypes.edges = varargin{i+1};
                                    else
                                        error('Parameters in list do not match datatypes.');
                                    end

                                case nodeDataTypesWords
                                    [isOK,N] = testofdatatypes(obj.N,varargin{i+1});
                                    if isOK
                                        obj.DataTypes.nodes = varargin{i+1};
                                    else
                                        error('Parameters in list do not match datatypes.');
                                    end
                ...
                % name nodes
                obj.N = torsche.graph.setnames(obj.N,'T');
                return;
            end

        %--------------------------------------------------------------------------
            % from taskset
            if nargin >= 1 && isa(varargin{1},'torsche.taskset')

                [E,N,eps,T,...
                 conversion_function1,conversion_param1,...
                 conversion_function2,conversion_param2] = torsche.graph.fromtaskset(varargin{:});
            ...
            elseif nargin >= 1 && isa(varargin{1},'double')
                ...
                for i = start:2:length(varargin)
                    switch lower(varargin{i})
                        ...
                        case 'color'
                            if torsche.iscolor(varargin{i+1}),
                                color = varargin{i+1};
                            else
                ...
                [E,N,eps] = torsche.graph.fromweightedadjmatrix(torsche.graph.testadjmatrix(varargin{1},emptyValue),emptyValue);
            ...
            % from edgeList or nodeList  
                ...
                if mod(length(varargin),2) ~= 0
                    ...
                    if isa(varargin{1}, 'struct')
                        ...
                        inumEdges = size(varargin{1}.eps,1);
                        imatrix = zeros(length(varargin{1}.N));
                        for i = 1:inumEdges
                            imatrix(varargin{1}.eps(i,1),varargin{1}.eps(i,2)) = imatrix(varargin{1}.eps(i,1),varargin{1}.eps(i,2)) + 1;
                        end
                        [E, N, eps] = torsche.graph.fromadjmatrix(torsche.graph.testadjmatrix(imatrix));
                        for i = 1:length(varargin{1}.N)
                            N(i).GraphicParam = varargin{1}.N(i).GraphicParam;
                        end
                        for i = 1:length(varargin{1}.E)
                            E(i).UserParam = varargin{1}.E(i).UserParam;
                        end
                        userparam = varargin{1}.UserParam;
                        color = varargin{1}.Color;
                        gridfreq = varargin{1}.GridFreq;
                    else
                        error('Arguments must come param/value in pairs.')
                    end
                    ...
                    for i = 1:2:length(varargin)                
                        switch lower(varargin{i})
                            case 'adj'
                                [E,N,eps] = torsche.graph.fromadjmatrix(torsche.graph.testadjmatrix(varargin{i+1}));

                            case 'inc'
                                [E,N,eps] = torsche.graph.fromincmatrix(torsche.graph.testincmatrix(varargin{i+1}));
                    ...
                    if ~isempty(edgeList) || ~isempty(nodeList)
                        [E,N,eps] = torsche.graph.fromedgeandnodelist(edgeList,nodeList);
...
%                     'GridFreq',gridfreq);
            ...
            obj.DataTypes = struct('edges',{cellOfEdgeDataTypes},...
                               'nodes',{cellOfNodeDataTypes});
            obj.Color = color;
            obj.GridFreq =gridfreq;

        %--------------------------------------------------------------------------        
            % Add parameters for edge (used in conversion taskset to graph)
            if exist('T','var')
                try
                    obj = feval(conversion_function2,obj,T.TSUserParam.EdgesParam,conversion_param2);
                catch
                    rethrow(lasterror)
                end
            end
            ...
            try
                obj.UserParam{end+1}.graphedit.nodeparams = conversion_param1;
            catch
            end
        end
    ...
    methods(Static)
        function [E,N,eps] = fromedgeandnodelist(edgeList,nodeList)
            ...
            if ~isempty(edgeList) && isempty(nodeList)
                try
                    N(1:max([edgeList{:,1:2}])) = torsche.node;
                    [E,eps] = torsche.graph.createedges(edgeList);
                    return;
                catch
                    wrongList = 'of edges';
                end
            ...
            elseif isempty(edgeList) && ~isempty(nodeList)
                try
                    N = torsche.graph.createnodes(nodeList);
                    E = []; eps = [];                % no edges
                    return;
                catch
                    wrongList = 'of nodes';
            ...
            elseif ~isempty(edgeList) && ~isempty(nodeList)        
                try
                    N = torsche.graph.createnodes(nodeList, max([edgeList{:,1:2}]));
                    [E,eps] = torsche.graph.createedges(edgeList);
                    return;
                catch
        ...
        function N = createnodes(nodeList, varargin)
            ...
            N(1:num_nodes2) = torsche.node;        % creating nodes
            for i = 1 : num_nodes
        ...
        function [E,N,eps,T,...
            ...
            na = nargin;
            T = varargin{1};
            [E,N,eps] = torsche.graph.fromadjmatrix(torsche.graph.testadjmatrix(T.Prec));
        ...
        function g = addnodes(g,nodeList)
            ...
            if ~isempty(nodeList)
                ...
                newNodes = torsche.graph.createnodes(nodeList);
                newNodes = torsche.graph.addemptyparams(newNodes,[nodeList{:,1}],nodeEmptyValues);
            end
        ...
        function g = addedges(g,edgeList)
            ...
            if ~isempty(edgeList)
                ...
                if maxNode > numNodesInGraph
                    ...
                    newNodes(1:(maxNode-numNodesInGraph)) = torsche.node;
                    newNodes = torsche.graph.addemptyparams(newNodes,[],nodeEmptyValues);
                else
                    newNodes = [];
                end
                [newEdges,newEps] = torsche.graph.createedges(edgeList);
            end
