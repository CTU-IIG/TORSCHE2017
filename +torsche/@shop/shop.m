classdef shop < torsche.schedobj
    properties
        Jobs
        TransportRobots
        LimitedBuffers
    ...
    methods
        function shopout = shop(varargin)
        ...
        % Suite 330, Boston, MA 02111-1307 USA
         ...
         if nargin==3
            if isa(varargin{1},'torsche.shop') && ischar(varargin{2}) 
                shopout = set_helper(varargin{:});
                return;
            end
        end
        ...
        shopout.Jobs=[];
        shopout.TransportRobots = [];
        shopout.LimitedBuffers = [];
        shopout.Schedule = 0;%Information whether schedule was computed.
        shopout.Type = 'none';
        shopout.parent = torsche.schedobj;
        ...
        if nargin == 2
            if isnumeric(varargin{1}) && isnumeric(varargin{2})
                ...
                for i=1:jobs
                    shopout.Jobs{i}=torsche.job(varargin{1}(i,:),varargin{2}(i,:));
                    shopout.Jobs{i}.Name = ['T_{' int2str(i) '}'];
                end	
        ...
        %%input is cell of jobs..
            ...
            if  nargin==2 || nargin==3
                if(isa(varargin{2},'torsche.limitedbuffers'))
                    shopout.LimitedBuffers = varargin{2};
                elseif isa(varargin{2},'torsche.transportrobots')
                    shopout.TransportRobots = varargin{2};
                else
            ...
            if nargin==3
                if(isa(varargin{3},'torsche.limitedbuffers'))
                    shopout.LimitedBuffers = varargin{3};
                elseif isa(varargin{3},'torsche.transportrobots')
                    shopout.TransportRobots = varargin{3};
                else
