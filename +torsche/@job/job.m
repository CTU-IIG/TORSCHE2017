classdef job < torsche.schedobj
    ...
    methods
        ...
        if nargin==3
            if isa(varargin{1},'torsche.job') && ischar(varargin{2}) 
                jobout = set_helper(varargin{:});
                return;
            end
        end
        ...
        if nargin==2 && nargout == 2
            ...
            jobout.parent = torsche.taskset;
            jobout.Name = '';
            jobout.ReleaseTime = 0;
            jobout.DeadLine = NaN;
            jobout.DueDate = NaN;
            jobout.Weight = 1;
            jobout.Version = 0.1;
                ...
                case 1 %input is taskset
                    if isa(varargin{1},'torsche.taskset')
                        for i=1:max(size(varargin{1}))
                            jobout.parent.tasks(i) = varargin{1}.tasks(i);
                        end
                    else
                ...
                case 2 % inputs are procTime and processor matrices 
                    ...
                    if isvector(procTime) && isvector(processor) && isnumeric(procTime) && isnumeric(processor) && max(size(procTime))==max(size(processor))
                        for i=1:max(size(procTime))
                            t = torsche.task(procTime(i));
                            t.Name = [jobout.Name, '_{', int2str(i), '}'];
                            t.Processor = processor(i);
                            jobout.parent.tasks(i) = t;
                        end
                    else
                        error('TORSCHE:Job:InvalidParam','Invalid parameters, see help!');
                    end
